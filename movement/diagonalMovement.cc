...
vector<PossibleMove> &DiagonalMovement::getPossibleMoves(const vector<vector<unique_ptr<Piece>>> &board, vector<PossibleMove> &allMoves, pair<int, int> position)
{
	...
	for (int i = 1; y + i < static_cast<int>(board[0].size()) && x + i < static_cast<int>(board.size()); i++)
	{
		if (board[x + i][y + i])
		{
	...
	for (int i = 1; 0 <= y - i && x + i < static_cast<int>(board.size()); i++)
	{
		if (board[x + i][y - i])
		{
			if (board[x + i][y - i]->getColor() != color)
	...
	for (int i = 1; y + i < static_cast<int>(board[0].size()) && 0 <= x - i; i++)
	{
		if (board[x - i][y + i])
		{
			if (board[x - i][y + i]->getColor() != color)
