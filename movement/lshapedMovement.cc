...
vector<PossibleMove>& LShapedMovement::getPossibleMoves(vector<PossibleMove>& allMoves, pair<int,int> position, int rows, int cols) {
	...
	for(auto v: deltas) {
		...
		if(0 <= newX && newX < rows && 0 <= newY && newY < cols) {
			res.push_back(PossibleMove{position, make_pair(newX, newY), true, true, false, true});
		}
	}
	return res;
};